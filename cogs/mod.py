...
class mod(commands.Cog):
    ...
    async def format_mod_embed(
        self, ctx, user, success, method, duration=None, location=None
    ):
        ...
        emb = discord.Embed()
        emb.set_author(
            name=method.title(), icon_url=user.avatar_url_as(static_format="png")
        )
    ...
    async def clearreaction(
        self, ctx, message: typing.Optional[int] = 1, emoji: discord.Emoji = None
    ):
    ...
    async def word(
        self, ctx, messages_to_delete: typing.Optional[int] = 10, *, words: str
    ):
